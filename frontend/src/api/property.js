...
export const propertyApi = {
  ...
  getFeeType: (id) => http.get(`/fee-types/${id}/`).then(unwrap),
  createFeeType: (payload) => http.post("/fee-types/", payload).then(unwrap),
  updateFeeType: (id, payload) => http.put(`/fee-types/${id}/`, payload).then(unwrap),
  getFeeTypeHistory: (id) => http.get(`/fee-types/${id}/history/`).then(unwrap),
